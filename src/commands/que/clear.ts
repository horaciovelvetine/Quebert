	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subCommand) =>
			subCommand
				...
				.addStringOption((opt) => opt.setName('post-id'))
				.setDescription('the posts id')
		)
		.addSubcommand((subCommand) => subCommand.setName('last').setDescription('remove the last post from the queue'))
		.addSubcommand((subCommand) => subCommand.setName('all').setDescription('remove all posts from the queue')),
